...
学习 Lean 最好的方法就是阅读和编写 Lean 代码。
本文将作为一个导读，介绍 Lean 语言的一些关键特性，并提供一些你可以在你的机器上执行的代码片段。
要了解关于设置开发环境的信息，请参阅 [设置 Lean](./setup.md)。
...
任何 Lean 程序最基本的部分是按命名空间组织的函数。
[函数](./functions.md) 对输入做操作，产生输出，它们被组织在[命名空间](./namespaces.md)下，这是你在 Lean 中分组的主要方式。
它们使用 [`def`](./definitions.md) 命令来定义，给函数定义一个名称并定义其参数。
